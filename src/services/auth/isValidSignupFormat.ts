...
export default (signupFormat: SignupFormat) => {
	...
	// Check if the email is an ibis-management email.
	if (!signupFormat.email.endsWith('ibis-management.com')) {
		alertTypeState.set('warning');
		alertTextState.set('Your Email does not appear to be an ibis-management email');
		return false;
	}
